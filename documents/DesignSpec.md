...
- ctsTraffic.exe (entrypoint)
  - `ctsTraffic.cpp` : program lifecycle, WSAStartup, configuration startup/shutdown, status timer, top-level broker startup and final summary.
- Configuration and logging
  - `ctsConfig.*` : parses command-line args, stores global `ctsConfig::g_configSettings`, exposes helper functions for printing, options, and socket setup.
...
- ctsIoPattern
  - Factory `MakeIoPattern()` selects concrete IO pattern implementation based on `ctsConfig::g_configSettings->IoPattern`.
...
- RIO / Registered IO: When using `WSA_FLAG_REGISTERED_IO` the pattern will use RIO buffers/IDs and adjust behavior to avoid extra buffer copies. See `ctsIOPattern.cpp` for multiple code-paths handling RIO vs overlapped IO.
