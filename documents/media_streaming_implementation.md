...
Implementation note: in the current code the `ctsMediaStreamSendRequests` constructor calls `ctl::ctTimer::snap_qpf()` to initialize its `m_qpf` member (so QPF is captured/assigned once per logical send request, and the helper itself uses an InitOnce to cache the OS frequency). The iterator's dereference operators (`operator->` and `operator*`) call `QueryPerformanceCounter` immediately before returning the WSABUF array, so the `m_qpcValue` is refreshed per-fragment.
...
Behavior on validation failure (implementation detail): the receive path uses `ctsMediaStreamMessage::ValidateBufferLengthFromTask` to perform these checks. When validation fails the implementation logs an error and returns a protocol error status (`TooFewBytes`) to the caller and the datagram is dropped. Note that in the current implementation this particular validation failure path logs an error but does not increment the global `UdpStatusDetails.m_errorFrames` counter; other error conditions (for example an out-of-range SequenceNumber) do increment the statistics counters elsewhere in the receive logic. Reviewers who need a strict diagnostics increment on header-validation failures should add an explicit counter increment at the validation-failure site in `ctsIOPatternMediaStream.cpp` or inside the `ValidateBufferLengthFromTask` helper.
...
Implementation note: helper functions exist that extract `SequenceNumber`, `Qpc`, and `Qpf` from incoming buffers (commonly named `GetSequenceNumberFromTask`, `GetQueryPerfCounterFromTask`, `GetQueryPerfFrequencyFromTask`). In the current codebase these helpers copy their data starting from `task.m_buffer + task.m_bufferOffset + c_udpDatagramProtocolHeaderFlagLength` (i.e., they respect `task.m_bufferOffset` and begin at the header's SequenceNumber start location). Concretely, `GetSequenceNumberFromTask` correctly copies the 8-byte SequenceNumber starting at header offset 2. However, `GetQueryPerfCounterFromTask` and `GetQueryPerfFrequencyFromTask` currently copy 8 bytes from the same start location as the sequence helper (also offset 2) rather than from the expected header offsets 10 and 18. This mismatch causes incorrect telemetry values when those helpers are used as-is.
...
- Receiver helpers (observed behavior):
 ...
 - Receiver consumers (observed legacy reads):
   - Some higher-level consumers perform direct reads using pointer casts instead of the helpers. For example, the receive-side jitter/telemetry logic performs reads like `*reinterpret_cast<int64_t*>(task.m_buffer + 8)` and `*reinterpret_cast<int64_t*>(task.m_buffer + 16)`. These reads access fixed byte offsets 8 and 16 relative to `task.m_buffer` and do not use `task.m_bufferOffset`. They also do not align with the authoritative header offsets for QPC and QPF (10 and 18). As a result, such consumers can observe incorrect or misaligned timing values, particularly when `task.m_bufferOffset` is non-zero.
