...
namespace ctsTraffic
{
    ...
    void ctsMediaStreamSenderListener(const std::weak_ptr<ctsSocket>& weakSocket) noexcept try
    {
        ctsMediaStreamSenderImpl::InitOnce();
        // ctsMediaStreamSenderImpl will complete the ctsSocket object
        // when a client request comes in to be 'accepted'
        ctsMediaStreamSenderImpl::AcceptSocket(weakSocket);
    }
    catch (...)
    {
    ...
    void ctsMediaStreamSenderIo(const std::weak_ptr<ctsSocket>& weakSocket) noexcept
    {
        ...
        try
        {
            ...
            do
            {
                ...
                {
                    ctsMediaStreamSenderImpl::ScheduleIo(weakSocket, nextTask);
                }
    ...
    void ctsMediaStreamSenderClose(const std::weak_ptr<ctsSocket>& weakSocket) noexcept try
    {
        ctsMediaStreamSenderImpl::InitOnce();

        if (const auto sharedSocket = weakSocket.lock())
        {
            ctsMediaStreamSenderImpl::RemoveSocket(sharedSocket->GetRemoteSockaddr());
        }
    }
    catch (...)
    {
    ...
    void ctsMediaStreamSenderClientStart(const std::weak_ptr<ctsSocket>& weakSocket) noexcept
    {
        ...
        const auto localAddr = sharedSocket->GetLocalSockaddr();
        const auto remoteAddr = sharedSocket->GetRemoteSockaddr();

        // construct a START message and send it synchronously via WSASendTo
        const ctsTask startTask = ctsMediaStreamMessage::Construct(MediaStreamAction::START);
        WSABUF wsaBuf{};
        wsaBuf.buf = reinterpret_cast<char*>(startTask.m_buffer);
        wsaBuf.len = static_cast<ULONG>(startTask.m_bufferLength);
        ...
        if (SOCKET_ERROR == sendResult)
        {
            ...
            ctsConfig::PrintErrorInfo(L"WSASendTo (START) failed [%d]", error);
            sharedSocket->CompleteState(error);
            return;
        }

        // Reuse server-side processing for START so that the connected socket is created
        try
        {
            ctsMediaStreamSenderImpl::InitOnce();
            ctsMediaStreamSenderImpl::Start(socket, localAddr, remoteAddr);
        }
        catch (...)
        {
            const auto error = ctsConfig::PrintThrownException();
    ...
    namespace ctsMediaStreamSenderImpl
    {
        ...
        static BOOL CALLBACK InitOnceImpl(PINIT_ONCE, PVOID, PVOID*) noexcept try
        {
            ...
            for (const auto& addr : ctsConfig::g_configSettings->ListenAddresses)
            {
                if (ctsConfig::g_configSettings->EnableRecvSharding)
                {
                    ...
                    for (uint32_t shard = 0; shard < shardCount; ++shard)
                    {
                        ...
                        if (error != NO_ERROR)
                        {
                            THROW_WIN32_MSG(error, "SetPreBindOptions (ctsMediaStreamSender)");
                        }
                        ...
                        if (maybeAffinities && shard < static_cast<uint32_t>(maybeAffinities->size()))
                        {
                            ...
                            {
                                PRINT_DEBUG_INFO(L"ctsMediaStreamSender - applied SIO_CPU_AFFINITY (group %u, cpu %u) to socket %Iu\n", ga.Group, processorIndex, listeningSocketToPrint);
                            }
                        }
                        ...
                        if (SOCKET_ERROR == bind(listening.get(), addr.sockaddr(), addr.length()))
                        {
                            ...
                            THROW_WIN32_MSG(error, "bind %hs (ctsMediaStreamSender)", addrBuffer);
                        }
                        ...
                        PRINT_DEBUG_INFO(
                            L"\t\tctsMediaStreamSender - Receiving datagrams on %ws (shard %d, %Iu)\n",
                ...
                else
                {
                    ...
                    PRINT_DEBUG_INFO(
                        L"\t\tctsMediaStreamSender - Receiving datagrams on %ws (%Iu)\n",
                        addr.writeCompleteAddress().c_str(),
                        listeningSocketToPrint);
                }
            }
            ...
            {
                throw std::exception("ctsMediaStreamSender invoked with no listening addresses specified");
            }
        ...
        void InitOnce()
        {
            ...
            {
                throw std::runtime_error("ctsMediaStreamSenderListener could not be instantiated");
            }
        }
            ...
            std::shared_ptr<ctsMediaStreamServerConnectedSocket> sharedConnectedSocket;
            {
                ...
                if (foundSocket == std::end(g_connectedSockets))
                {
                    ctsConfig::PrintErrorInfo(
                        L"ctsMediaStreamSender - failed to find the socket with remote address %ws in our connected socket list to continue sending datagrams",
            ...
            if (const auto sharedSocket = weakSocket.lock())
            {
                ...
                else
                {
                    ...
                    if (existingSocket != std::end(g_connectedSockets))
                    {
                        ...
                        PRINT_DEBUG_INFO(L"\t\tctsMediaStreamSender::accept_socket - socket with remote address %ws asked to be Started but was already established",
                    ...
                    PRINT_DEBUG_INFO(L"\t\tctsMediaStreamSender::accept_socket - socket with remote address %ws added to connected_sockets",
                        waitingEndpoint->second.writeCompleteAddress().c_str());
        ...
        void Start(SOCKET socket, const ctl::ctSockaddr& localAddr, const ctl::ctSockaddr& targetAddr)
        {
            ...
            {
                ctsConfig::g_configSettings->UdpStatusDetails.m_duplicateFrames.Increment();
                PRINT_DEBUG_INFO(L"\t\tctsMediaStreamSender::start - socket with remote address %ws asked to be Started but was already in connected_sockets",
            ...
            {
                ctsConfig::g_configSettings->UdpStatusDetails.m_duplicateFrames.Increment();
                PRINT_DEBUG_INFO(L"\t\tctsMediaStreamSender::start - socket with remote address %ws asked to be Started but was already in awaiting endpoints",
                    targetAddr.writeCompleteAddress().c_str());
                // return early if this was a duplicate request: this can happen if there is latency or drops
                // between the client and server as they attempt to negotiating starting a new stream
                return;
            ...
            if (addToAwaiting)
            {
                PRINT_DEBUG_INFO(L"\t\tctsMediaStreamSender::start - socket with remote address %ws added to awaiting_endpoints",
                    targetAddr.writeCompleteAddress().c_str());
