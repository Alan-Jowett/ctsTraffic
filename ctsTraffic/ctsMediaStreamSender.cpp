...
ctsMediaStreamSender::~ctsMediaStreamSender() noexcept
{
    // stop the TP before letting the destructor delete any member objects
    m_taskTimer.reset();
}

void ctsMediaStreamSender::ScheduleTask(const ctsTask& task) noexcept
{
...
void ctsMediaStreamSender::CompleteState(uint32_t errorCode) const noexcept
{
    if (const auto sharedSocket = m_weakSocket.lock())
    {
...
VOID CALLBACK ctsMediaStreamSender::MediaStreamTimerCallback(PTP_CALLBACK_INSTANCE, PVOID context, PTP_TIMER) noexcept
{
    auto* thisPtr = static_cast<ctsMediaStreamSender*>(context);
    ...
    if (ctsIoStatus::FailedIo == status)
    {
        ...
        ctsConfig::PrintErrorInfo(
            L"MediaStream Sender socket (%ws) was indicated Failed IO from the protocol - aborting this stream",
    ...
    {
        PRINT_DEBUG_INFO(
            L"\t\tctsMediaStreamSender socket (%ws) has completed its stream - closing this 'connection'\n",
            thisPtr->m_remoteAddr.writeCompleteAddress().c_str());
